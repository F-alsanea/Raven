...
`packages/core/system-prompt/tests/raven-fable.spec.ts` pins complete-prompt assembly, runtime-context suppression, tool-schema preservation, the source SHA, and plugin disposal. `packages/core/tools/tests/raven-fable-compat.spec.ts` pins adapter argument translation, target-driven registration/removal, policy-preserving nested dispatch, and plugin disposal. The shipped headless keyless smoke boots the real Loader tree, reads the persisted `system/message`, requires exact equality with the vendored prompt, and requires `web_search_fast` in the model-facing tool roster. Adoption also verifies source and vendored Git blob SHA equality.
